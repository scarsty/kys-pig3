...
function CommonMenu(x, y, w, max, default: integer; menuString, menuEngString: array of utf8string; needFrame: integer; color1, color2, menucolor1, menucolor2: uint32): integer; overload;
...
function CommonMenu2(x, y, w: integer; menuString: array of utf8string; max: integer = 1): integer; overload;
function SelectOneTeamMember(x, y: integer; str: utf8string; list1, list2: integer; mask: integer = 63): integer;
procedure MenuEsc;
...
begin
  ...
  str := SDL_GetAndroidExternalStoragePath() + '/pig3_place_game_here';
  ...
  SDL_RenderTexture(render, logo, nil, @rect);
  SDL_RenderPresent(render);
...
procedure Quit;
begin
  if (SDL_JoystickConnected(joy)) then
...
begin
  ...
  begin
    ...
    Star[0] := list.Strings[0];
    if list.Count >= 216 then
      for i := 1 to 107 do
      begin
        //ReadTalk(i, NameStr);
        RoleName[i] := list.Strings[i + 108];
        Star[i] := list.Strings[i];
      end;
  end
  else
    for i := 0 to 107 do
...
begin
  ...
  begin
    ...
    repeat
      ...
      ShowStatus(0);
      //showmessage('');
      DrawTextWithRect(str2, 150, CENTER_Y + 120, 80, 0, $202020, 30, 0);
...
begin
  ...
  else if Rrole[0].Aptitude > 60 then
  begin
    r := random(8);
...
function LoadR(num: integer): boolean;
var
  zfilename, filenamer, filenames, filenamed, s: utf8string;
  str, p, p1: putf8char;
  key1, key2: pbyte;
...
begin
  ...
  begin
    ...
    begin
      ...
      FileRead(GRP, Sdata, SceneAmount * 64 * 64 * 6 * 2);
      FileClose(GRP);
      GRP := FileOpen(AppPath + 'save/' + filenamed, fmopenread);
      FileRead(GRP, Ddata, SceneAmount * 200 * 11 * 2);
      FileClose(GRP);
    end
    else
    begin
  ...
  begin
    ...
    if (num = 0) or (MODVersion = 13) or (MODVersion = 31) then
      BufferRead(p1, @Ritem[0], SceneOffset - ItemOffset)
    else
      p1 := p1 + SceneOffset - ItemOffset;
    BufferRead(p1, @Rscene[0], MagicOffset - SceneOffset);
  ...
  begin
    ...
    move(Rscene[low(Rscene)], Rscene0[low(Rscene0)], sizeof(Tscene) * length(Rscene));
  ...
  {if Rrole[166].HeadNum <> 16 then
    ...
    for i1 := 0 to high(Rscene) do
    begin
    big5tou16(@Rscene[i1].Name[0]);
    end;
...
begin
  ...
  BufferWrite(p1, @Ritem[0], SceneOffset - ItemOffset);
  BufferWrite(p1, @Rscene[0], MagicOffset - SceneOffset);
  ...
  begin
    ...
    begin
      FileWrite(GRP, Sdata, SceneAmount * 64 * 64 * 6 * 2);
      FileClose(GRP);
    end;
    ...
    begin
      FileWrite(GRP, Ddata, SceneAmount * 200 * 11 * 2);
      FileClose(GRP);
    end;
  end
  else
  begin
    ...
    begin
      ...
      zipWriteInFileInZip(zFile, @Sdata[0, 0, 0, 0], SceneAmount * 64 * 64 * 6 * 2);
      zipCloseFileInZip(zfile);
      zipOpenNewFileInZip(zFile, putf8char(filenamed), nil, nil, 0, nil, 0, nil, Z_DEFLATED, Z_DEFAULT_COMPRESSION);
      zipWriteInFileInZip(zFile, @Ddata[0, 0, 0], SceneAmount * 200 * 11 * 2);
      zipCloseFileInZip(zfile);
      zipClose(zfile, nil);}
      zip_compress(z, putf8char(filenamer), p, lenR);
      zip_compress(z, putf8char(filenames), @Sdata[0, 0, 0, 0], SceneAmount * 64 * 64 * 6 * 2);
      zip_compress(z, putf8char(filenamed), @Ddata[0, 0, 0], SceneAmount * 200 * 11 * 2);
      zip_Close(z);
    end;
  end;
  StrDispose(p);
...
begin
  while True do
  begin
    if (where = 1) and (CurEvent < 0) and (not LoadingScene) and (NeedRefreshScene = 1) then
      InitialScene(2);
...
var
  ...
  Sx1, Sy1, s, i, walking, Prescene, stillcount, speed: integer;
...
begin
  ...
  begin
    ...
    if ((Sx = Rscene[CurScene].JumpX1) and (Sy = Rscene[CurScene].JumpY1)) and (Rscene[CurScene].JumpScene >= 0) then
    begin
      instruct_14;
      PreScene := CurScene;
    ...
    case event.type_ of
      ...
      begin
        ...
        if (event.key.key = SDLK_TAB) then
        begin
          SpecialFunction;
        end;
      end;
        ...
        begin
          ...
          begin
            ...
            {if (axp < 0) or (axp > 63) or (ayp < 0) or (ayp > 63) then
              begin
              axp := Rscene[CurScene].ExitX[0];
              ayp := Rscene[CurScene].ExitY[0];;
              end;}
            if (axp >= 0) or (axp <= 63) or (ayp >= 0) or (ayp <= 63) then
            begin
              FillChar(Fway[0, 0], sizeof(Fway), -1);
              ...
              begin
                for i1 := axp + 1 downto axp do
                  ...
                  begin
                    if (SData[CurScene, 3, i1, i2] > 0) and not (CanWalkInScene(i1, i2)) then
                    begin
                      axp := i1;
                      ayp := i2;
                    end;
  ...
  MFace := SFace;
  resetEntrance();
  if (exitscenemusicnum > 0) and (where <> 3) then
  begin
    StopMP3;
...
begin
  UpdateAllScreen;
  //显示场景名
  scenename := Rscene[snum].Name;
  //c:=sdl_maprgba(screen.format,0,255,0,0);
  DrawTextWithRect(scenename, CENTER_X - DrawLength(scenename) * 5 - 23, 100, 0, 0, $202020);
  SDL_Delay(500);
...
function CanWalkInScene(x, y: integer): boolean; overload;
begin
  Result := CanWalkInScene(Sx, Sy, x, y);
...
begin
  ...
  begin
    if (SData[CurScene, 1, x, y] <= 0) and (SData[CurScene, 1, x, y] >= -2) then
      ...
      Result := False;
    if (abs(SData[CurScene, 4, x, y] - SData[CurScene, 4, x1, y1]) > 10) and (abs(x1 - x) + abs(y1 - y) = 1) then
      Result := False;
...
begin
  ...
  begin
    ...
    begin
      //writeln(DData[CurScene, SData[CurScene, 3, x, y], 2]);
      Cx := Sx;
      Cy := Sy;
      Sstep := 0;
      CurSceneRolePic := BEGIN_WALKPIC2 + SFace * 7 + SStep;
  ...
  CurEvent := -1;
  if (MMAPAMI = 0) or (SCENEAMI = 0) then
  begin
    Redraw;
    UpdateAllScreen;
  end;
...
{
  function CommonMenu(x, y, w, max, default: integer; menuString, menuEngString: array of utf8string;
...
var
  ...
  begin
    ...
    begin
      ...
      if p = 1 then
        DrawEngShadowText(menuEngString[i], x + 19 + (len + 1) * 20, y + 3 + h * (i - menutop), c1, c2);
    end;
  end;
...
begin
  ...
  if str <> '' then
  begin
    DrawTextWithRect(str, CENTER_X - 275, CENTER_Y - 193, 0, 0, $202020);
  end;
  ...
  begin
    ...
    CheckBasicEvent;
    if ((event.type_ = SDL_EVENT_KEY_DOWN) and (event.key.key = SDLK_LEFT)) then
    begin
      menu := menu - 1;
      if menu < 0 then
        menu := max;
    end;
    if ((event.type_ = SDL_EVENT_KEY_DOWN) and (event.key.key = SDLK_RIGHT)) then
    begin
      menu := menu + 1;
      if menu > max then
        menu := 0;
    end;
    if ((event.type_ = SDL_EVENT_KEY_DOWN) and (event.key.key = SDLK_UP)) then
    begin
    ...
    if ((event.type_ = SDL_EVENT_KEY_DOWN) and (event.key.key = SDLK_DOWN)) then
    begin
      menu := menu + 2;
      if menu > max then
        menu := menu - max - 1;
...
begin
  ...
  begin
    ...
    menup := menu;
    info := format('位置：(%3d, %3d)', [My, Mx]);
    DrawShadowText(info, 5, 5, ColColor($64), ColColor($66));
    UpdateAllScreen;
    CheckBasicEvent;
    case event.type_ of
      SDL_EVENT_KEY_DOWN:
      ...
      begin
        ...
        if (event.key.key >= SDLK_1) and (event.key.key <= SDLK_4) then
        begin
...
begin
  Result := MenuEscType;
  if (event.type_ = SDL_EVENT_MOUSE_BUTTON_UP) and (event.button.button = SDL_BUTTON_LEFT) and MouseInRegion(TitleMenu[0].x, TitleMenu[0].y, TitleMenu[3].x + TitleMenu[3].w - TitleMenu[0].x, TitleMenu[3].y + TitleMenu[3].h - TitleMenu[0].y) then
  begin
    for i := 0 to 3 do
    begin
  ...
  if (event.type_ = SDL_EVENT_KEY_UP) and (event.key.key >= SDLK_1) and (event.key.key <= SDLK_4) then
  begin
...
begin
  str := ('隊員醫療能力');
  DrawTextWithRect(str, 80, 30, 132, ColColor($23), ColColor($21));
...
var
  ...
  begin
    ...
    begin
      ...
      //DrawShadowText(str, x * d + 5 + xp + 19, y * d + 35 + yp + 85, ColColor($64), ColColor($66));
    ...
    begin
      ...
      begin
        str := '你的位置：';
        DrawShadowText(str, 8 + xp, 48 + dt + yp, 0, $202020);
        str := format('%3d,%3d', [My, Mx]);
        DrawShadowText(str, 108 + xp, 48 + dt + yp, ColColor($64), ColColor($66));

        str := '船的位置：';
        DrawShadowText(str, 188 + xp, 48 + dt + yp, 0, $202020);
        str := format('%3d,%3d', [Shipx, shipy]);
        DrawShadowText(str, 288 + xp, 48 + dt + yp, ColColor($64), ColColor($66));
      end
      else
      begin
    ...
    begin
      ...
      begin
        str := '功效：';
        DrawShadowText(str, 8 + xp, 78 + dt + yp, ColColor($21), ColColor($23));
      end;
      if len3 > 0 then
      begin
        str := '需求：';
        DrawShadowText(str, 8 + xp, 78 + dt + (len2 + l1) div l * 28 + yp, ColColor($21), ColColor($23));
      end;
      ...
      begin
        ...
        begin
          ...
          DrawShadowText(words2[i], 68 + i1 mod l * w + xp, i1 div l * 28 + 78 + dt + yp, ColColor(5), ColColor(7));
          DrawShadowText(str, 108 + i1 mod l * w + xp, i1 div l * 28 + 78 + dt + yp, color1, color2);
          i1 := i1 + 1;
        end;
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            //setfontsize(12, 19);
            str := '使用中';
            DrawTextWithRect(str, ui_x + 15, ui_y + i * 80 + 50, 0, ColColor($64), ColColor($66), 50, 0);
            //DrawMPic(2020, CENTER_X - 384 + 15, CENTER_Y - 240 + 80 * i + 50, 0, 0, 30);
          end;
          {if (menu = 2) and (Rrole[teamlist[i]].PracticeBook >= 0) then
            begin
            str := putf8char(@Ritem[Rrole[teamlist[i]].PracticeBook].Name);
            DrawShadowText(str, CENTER_X - 384 + 15, CENTER_Y - 240 + 80 * i + 60, ColColor($64), ColColor($66));
            end;}
          ...
          begin
            ...
            if level > 0 then
            begin
              str := format('%2d級', [level]);
              DrawShadowText(str, ui_x + 220, ui_y + 80 * i + 60, ColColor($64), ColColor($66));
            end;
          end;
        end;
      end;
      ...
      begin
        ...
        DrawShadowText(menuString[i], titlex1 + titlew * i + 12, titley1, color1, color2);
        //if i < titlemax then
        //DrawShadowText(str, titlex1 + titlew * i + 50, titley1, ColColor($7a), ColColor($7c));
      end;
      if dragitem >= 0 then
      begin
    ...
    case event.type_ of
      ...
      begin
        ...
        if (event.key.key = SDLK_RETURN) or (event.key.key = SDLK_SPACE) or ((event.key.key = SDLK_DOWN) and (intitle = 1)) then
        begin
          //ReDraw;
          if intitle = 0 then
          begin
  ...
  case Ritem[inum].ItemType of
    ...
    begin
      ...
      begin
        ...
        menuString[1] := '繼續';
        str := '此物品正有人裝備，是否繼續？';
        DrawTextWithRect(str, CENTER_X - 142, CENTER_Y - 40, 285, 0, $202020);
      ...
      begin
        ...
        begin
          ...
          DrawShadowText(str1, CENTER_X - 275 + 80 + off, CENTER_Y - 193 + 3, {160, 32,} ColColor($64), ColColor($66));
          UpdateAllScreen;
          mask := 0;
          for i := 0 to 5 do
          begin
        ...
        begin
          ...
          begin
            str := '此人不適合裝備此物品';
            DrawTextWithRect(str, CENTER_X - 100, CENTER_Y + 40, 205, ColColor($64), ColColor($66));
            WaitAnyKey;
            Redraw;
            //SDL_UpdateRect2(screen,0,0,screen.w,screen.h);
          end;
...
begin
    ...
    begin
      ...
      begin
        ...
        begin
          case Rmagic[mnum].HurtType of
            3:
              if HaveMagicAmount(rnum, 1) >= 4 then
                Result := False;
            0, 1, 2:
              if HaveMagicAmount(rnum) >= 10 then
                Result := False;
          end;
        end;
      end;
  ...
  if (MODVersion <> 13) and (use = 1) then
    ...
    begin
      ...
      menuString[1] := ('繼續');
      str := ('是否要自宮？');
      DrawTextWithRect(str, CENTER_X - 63, CENTER_Y, 0, 0, $202020);
...
{procedure MenuStatus;
  ...
  begin
  str := '查看隊員狀態';
  drawtextwithrect(str, 80, 30, 132, colcolor($21), colcolor($23));
...
begin
  ...
  begin
    ...
    {dest.y := 80 * menu + yeff;
      ...
      f := f + 1;}
    CheckBasicEvent;
    if ((event.type_ = SDL_EVENT_KEY_DOWN) and (event.key.key = SDLK_UP)) or ((event.type_ = SDL_EVENT_MOUSE_WHEEL) and (event.wheel.y > 0)) then
    begin
      menu := menu - 1;
      if menu < 0 then
        menu := max;
    end;
    if ((event.type_ = SDL_EVENT_KEY_DOWN) and (event.key.key = SDLK_DOWN)) or ((event.type_ = SDL_EVENT_MOUSE_WHEEL) and (event.wheel.y < 0)) then
    begin
      menu := menu + 1;
      if menu > max then
        menu := 0;
    end;
    if (event.type_ = SDL_EVENT_KEY_DOWN) and ((event.key.key = SDLK_LEFT) or (event.key.key = SDLK_RIGHT)) then
    begin
      equip := 1 - equip;
    end;
    if ((event.type_ = SDL_EVENT_KEY_UP) and (event.key.key = SDLK_ESCAPE)) or ((event.type_ = SDL_EVENT_MOUSE_BUTTON_UP) and (event.button.button = SDL_BUTTON_RIGHT)) then
    ...
    if ((event.type_ = SDL_EVENT_MOUSE_BUTTON_UP) and (event.button.button = SDL_BUTTON_LEFT) and MouseInRegion(item1x, item1y, d, d)) or ((event.type_ = SDL_EVENT_KEY_UP) and (equip = 0) and ((event.key.key = SDLK_RETURN) or (event.key.key = SDLK_SPACE))) then
    begin
    ...
    if ((event.type_ = SDL_EVENT_MOUSE_BUTTON_UP) and (event.button.button = SDL_BUTTON_LEFT) and MouseInRegion(item2x, item2y, d, d)) or ((event.type_ = SDL_EVENT_KEY_UP) and (equip = 1) and ((event.key.key = SDLK_RETURN) or (event.key.key = SDLK_SPACE))) then
    begin
...
var
  ...
  str: utf8string;
  strs: array [0 .. 24] of utf8string;
  //strs1: array of utf8string;
  color1, color2: uint32;
  Name: utf8string;
...
begin
  ...
  begin
    ...
    //显示所需字符
    for i := 0 to 5 do
      DrawTextWithRect(strs[i], x - 10, y + 208 + h * i, 140, 0, $202020, 30, 0);

    //等级
    str := format('%4d', [Rrole[rnum].Level]);
    DrawEngShadowText(str, x + 110, y + 211 + h * 0, ColColor($64), ColColor($66));
    ...
    DrawEngShadowText(str, x + 60, y + 211 + h * 1, color1, color2);

    str := '/';
    DrawEngShadowText(str, x + 100, y + 211 + h * 1, ColColor($64), ColColor($66));
    ...
    DrawEngShadowText(str, x + 110, y + 211 + h * 1, color1, color2);
    //内力, 依据内力性质使用颜色
    ...
    DrawEngShadowText(str, x + 60, y + 211 + h * 2, color1, color2);
    //体力
    str := format('%4d/%4d', [Rrole[rnum].PhyPower, MAX_PHYSICAL_POWER]);
    DrawEngShadowText(str, x + 60, y + 211 + h * 3, ColColor($64), ColColor($66));
    //经验
    str := format('%5d', [uint16(Rrole[rnum].Exp)]);
    DrawEngShadowText(str, x + 100, y + 211 + h * 4, ColColor($64), ColColor($66));
    str := format('%5d', [uint16(Leveluplist[Rrole[rnum].Level - 1])]);
    DrawEngShadowText(str, x + 100, y + 211 + h * 5, ColColor($64), ColColor($66));

    //str:=format('%5d', [Rrole[rnum,21]]);
    //drawengshadowtext(str,150,295,colcolor($7),colcolor($5));
    ...
    //drawengshadowtext(str, 150, 341, colcolor($14), colcolor($16));
    //str := format('%4d', [RRole[rnum].Poison]);
    //drawengshadowtext(str, 150, 362, colcolor($35), colcolor($37));
    ...
    begin
      ...
      y := yp;
      //装备, 秘笈
      DrawTextWithRect(strs[18], item1x + 85, item1y, 0, 0, $202020, 0, 0);
  ...
  begin
    ...
    begin
      ...
      //display_img(putf8char(AppPath + 'resource/lu.png'), x + 270 - 10, y - 38);
      for i := 0 to 2 do
      begin
        DrawTextWithRect(strs[i], x + 180 + 100, y + 2 + h * i, 240, 0, $202020, 40, 0);
      end;
      for i := 0 to 14 do
      begin
        str := '->';
        DrawShadowText(str, x + 300 + 100 + 50, y + 5 + h * i, ColColor($64), ColColor($66));
      end;
    end;
    str := format('%4d', [Rrole[rnum].Level]);
    DrawEngShadowText(str, x + 280 + 100, y + 5 + h * 0, ColColor($64), ColColor($66));
    str := format('%4d', [Rrole[rnum].MaxHP]);
    DrawEngShadowText(str, x + 280 + 100, y + 5 + h * 1, ColColor($64), ColColor($66));
    str := format('%4d', [Rrole[rnum].MaxMP]);
    DrawEngShadowText(str, x + 280 + 100, y + 5 + h * 2, ColColor($64), ColColor($66));
    y := y + 3 * h;
  end;
  ...
  begin
    ...
    if bnum <> -2 then
    begin
      DrawTextWithRect(strs[i], x + 180 + 100, y + 2 + h * (i - 6), w, 0, $202020, 40, 0);
    end;
  end;
    ...
    begin
      ...
      begin
        if addnum[i] > 0 then
          ...
          str := format(' (%d)', [addnum[i]]);
        DrawEngShadowText(str, x + 280 + 140, y + 5 + i * h, color1, color2);
      end;
    end;
  end;
  color1 := ColColor($64);
  ...
  {//武功
    ...
    drawengshadowtext(str, x + 480, y + 26 + 21 * i, colcolor($64), colcolor($66));
    end;
    end;
    ...
    drawengshadowtext(str, x + 480, y + 26 + 260 + 21 * i, colcolor($64), colcolor($66));
    end;
    end;}
...
var
  ...
  strs1: array [1 .. 17] of utf8string;
...
begin
  ...
  begin
    ...
    begin
      ...
      SDL_FillSurfaceRect(SimpleState, nil, 0);
      //sur := SimpleState;
    end;
  end
  else
  ...
  str := format('%4d', [Rrole[rnum].CurrentHP]);
  DrawEngShadowText(str, x + 138, y + 28, color1, color2, tex, sur);

  str := '/';
  DrawEngShadowText(str, x + 165, y + 28, ColColor($64), ColColor($66), tex, sur);
  ...
  str := format('%4d', [Rrole[rnum].MaxHP]);
  DrawEngShadowText(str, x + 173, y + 28, color1, color2, tex, sur);

  //str:=format('%4d/%4d', [Rrole[rnum,17],Rrole[rnum,18]]);
  //drawengshadowtext(str,x+50,y+107,colcolor($7),colcolor($5));
  if Rrole[rnum].MPType = 0 then
  begin
    color1 := ColColor($4E);
    color2 := ColColor($50);
  ...
  DrawEngShadowText(str, x + 138, y + 44, color1, color2, tex, sur);

  str := '/';
  DrawEngShadowText(str, x + 165, y + 44, color1, color2, tex, sur);

  str := format('%4d', [Rrole[rnum].MaxMP]);
  DrawEngShadowText(str, x + 173, y + 44, color1, color2, tex, sur);

  str := format('%3d', [Rrole[rnum].PhyPower]);
  DrawEngShadowText(str, x + 148, y + 61, ColColor(5), ColColor(7), tex, sur);
...
var
  ...
  menustring2: array [0 .. 2] of utf8string;
  tempsur, tempsur2: PSDL_Surface;
  dest: TSDL_Rect;
  drag, xm, ym: integer;
  maxselect: integer;
begin
  ...
  begin
    ...
    if ((event.type_ = SDL_EVENT_KEY_UP) and ((event.key.key = SDLK_RETURN) or (event.key.key = SDLK_SPACE))) or ((event.type_ = SDL_EVENT_MOUSE_BUTTON_UP) and (event.button.button = SDL_BUTTON_LEFT) and (mouseactive = 1)) then
    begin
      if select >= 0 then
      begin
        ...
        begin
          TransBlackScreen;
          menu2 := SelectOneTeamMember(x + 50, y + 80 + select * 150, '選擇目標隊友', 0, 0);
          if menu2 >= 0 then
          begin
            case select of
...
var
  ...
  str: utf8string;
  strs: array [0 .. 23] of utf8string;
  strs1: array [0 .. 2] of utf8string;
  color1, color2: uint32;
begin
  strs[0] := '普通';
  strs[1] := '武學';
  ...
  DrawTextWithRect(strs[0], x + 70, y + 20, 10, 0, $202020, 0, 0);
  ...
  begin
    ...
    //str := format('%4d', [Rrole[rnum].Medcine]);
    //DrawEngShadowText(str, x + 100, y + 50, ColColor($64), ColColor($66));
  ...
  begin
    ...
    //str := format('%4d', [Rrole[rnum].MedPoi]);
    //DrawEngShadowText(str, x + 250, y + 50, ColColor($64), ColColor($66));
  ...
  begin
    ...
    //DrawShadowText(strs1[2], x + 370, y + 50, color1, color2);
  end;

  //武功
  DrawTextWithRect(strs[1], x + 70, y + 90, 0, 0, $202020, 0, 0);
  color1 := 0;
  color2 := $202020;
  for i := 0 to 9 do
  begin
  ...
  //内功
  DrawTextWithRect(strs[2], x + 70, y + 270, 10, 0, $202020, 0, 0);
  color1 := 0;
  color2 := $202020;
  for i := 0 to 3 do
  begin
  ...
  DrawTextWithRect(strs[3], x + 70, y + 370, 0, 0, $202020, 0, 0);
  ...
  begin
    ...
    DrawTextWithRect(str, x + 70, y + 428, 0, ColColor($64), ColColor($66), 20, 0);
...
begin
  ...
  //menu := SelectOneTeamMember(80, 65, '%3d', 15, 0);
  TransBlackScreen;
  menu := SelectOneTeamMember(CENTER_X - 384 + 270 + 50, CENTER_Y - 240 + 7 + 80, '要求誰離隊', 0, 0);
  if menu >= 0 then
  begin
    for i := 0 to 99 do
...
begin
  ...
  begin
    ...
    begin
      ...
      //SDL_BlitSurface(tempsur, nil, screen, nil);
      if (TEXT_LAYER = 1) then
        SDL_FillSurfaceRect(TextScreen, nil, 0);
      if where <> 2 then
      begin
        for i := 0 to maxteam do
        begin
...
var
  ...
  filename: utf8string;
  str, str2: array [0 .. 7] of utf8string;
  menuString: array [0 .. 1] of utf8string;
...
var
  menu, i, x, y: integer;
  str: utf8string;
  menuString, menuEngString: array [0 .. 10] of utf8string;
  filename: utf8string;
begin
  x := CENTER_X;
  y := 90;
...
begin
  ...
  if Result = -2 then
  begin
    //DrawShadowText(str, CENTER_X + 40, CENTER_Y + 200, menucolor1, menucolor2);
    DrawTextWithRect(str, x + 170, y + 310, 0, menucolor1, menucolor2);
    UpdateAllScreen;
    WaitAnyKey;
  end;
...
begin
  ...
  //水浒中的71古墓场景不可存档
  if ((where = 1) and (MODVersion = 13) and (CurScene = 71)) or (ScreenBlendMode > 0) then
  begin
    str := '此時不可存檔！';
    //Redraw;
  ...
  begin
    ...
    if menu >= 0 then
      ...
      begin
        str := '存檔失敗！索引丟失或文件被佔用！';
        DrawTextWithRect(str, CENTER_X - 384 + 420 - 40, CENTER_Y - 240 + 90 + 280, 322, MapRGBA(240, 20, 20), MapRGBA(212, 20, 20));
        WaitAnyKey;
      end;
  end;
  //ShowMenu(3);
...
begin
  ...
  begin
    ...
    //str := '请按任意键继续…';
    //DrawShadowText(str, CENTER_X - 120, CENTER_Y - 25, ColColor(255), ColColor(255));
    //updateallscreen;
    //PlayMP3(StartMusic, -1);
    //WaitAnyKey;
    //Redraw;
  ...
  begin
    ...
    str := '輸入腳本文件名';
    DrawTextWithRect(str, CENTER_X - 80, CENTER_Y - 240 + 130, 148, 0, ColColor($23));
...
begin
  ...
  begin
    str := '無明顯效果';
    DrawTextWithRect(str, 14 + xp, 132 + yp, 10, ColColor($64), ColColor($66));
  end;
  p := 0;
  ...
  begin
    ...
    begin
      ...
      DrawShadowText(word[i], 33 + xp + x, 130 + yp + y + p * 28, 0, $202020);
      //if i <> 21 then
      str := format('%5d', [addvalue[i]]);
      //else
      //str := format('%5.1f', [addvalue[i] / 10]);
      DrawEngShadowText(str, 163 + xp + x, 130 + yp + y + p * 28, ColColor($64), ColColor($66));
      p := p + 1;
    end;
    //对内力性质特殊处理
...
begin
  {for i1 := 0 to 479 do
    ...
    Entrance[i1, i2] := -1;}
  FillChar(Entrance[0, 0], sizeof(Entrance), -1);
  for i := 0 to min(SceneAmount - 1, high(Rscene)) do
  begin
...
begin
  ...
  begin
    ...
    SDL_FillSurfaceRect(sur, nil, MapRGBA(0, 0, 0, 255));
  end;
    ...
    begin
      ...
      begin
        if TEXT_LAYER = 1 then
          ...
          SDL_SetRenderTarget(render, screenTex);
        SDL_RenderTexture(render, tex, @dest, nil);
      end
      else
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          SDL_RenderTexture(render, tex, @tempdest, @dest);
        end
        else
        begin
          if TEXT_LAYER = 0 then
