...
void instruct_2(int inum, int amount)
{
    ...
    std::string w2;
    if (amount >= 0)
        w2 = "\xe5\xbe\x97\xe5\x88\xb0"; // 得到
    else
    {
        w2 = "\xe5\xa4\xb1\xe5\x8e\xbb"; // 失去
        amount = -amount;
    }
...
{
    std::string menuString[2] = {"\xe5\x8f\x96\xe6\xb6\x88", "\xe6\x88\xb0\xe9\xac\xa5"}; // 取消, 戰鬥
    std::string str = "\xe6\x98\xaf\xe5\x90\xa6\xe8\x88\x87\xe4\xb9\x8b\xe6\x88\xb0\xe9\xac\xa5\xef\xbc\x9f"; // 是否與之戰鬥？
...
{
    std::string menuString[2] = {"\xe5\x8f\x96\xe6\xb6\x88", "\xe8\xa6\x81\xe6\xb1\x82"}; // 取消, 要求
    std::string str = "\xe6\x98\xaf\xe5\x90\xa6\xe8\xa6\x81\xe6\xb1\x82\xe5\x8a\xa0\xe5\x85\xa5\xef\xbc\x9f"; // 是否要求加入？
    DrawTextWithRect(str.c_str(), CENTER_X - 75, CENTER_Y - 85, 0, 0, 0x202020);
    std::vector<std::string> ms(menuString, menuString + 2);
    int menu = CommonMenu2(CENTER_X - 49, CENTER_Y - 50, 48, ms);
    int result = (menu == 1) ? jump1 : jump2;
...
int instruct_11(int jump1, int jump2)
{
    ...
    {
        menuString[0] = "N-\xe5\x90\xa6"; // N-否
        menuString[1] = "Y-\xe6\x98\xaf"; // Y-是
        str = "\xe8\xab\x8b\xe4\xbd\x9c\xe5\x87\xba\xe6\x82\xa8\xe7\x9a\x84\xe9\x81\xb8\xe6\x93\x87"; // 請作出您的選擇
    }
    else
    {
        menuString[0] = "\xe5\x8f\x96\xe6\xb6\x88"; // 取消
        menuString[1] = "\xe4\xbd\x8f\xe5\xae\xbf"; // 住宿
        str = "\xe6\x98\xaf\xe5\x90\xa6\xe9\x9c\x80\xe8\xa6\x81\xe4\xbd\x8f\xe5\xae\xbf\xef\xbc\x9f"; // 是否需要住宿？
    }
    DrawTextWithRect(str.c_str(), CENTER_X - 75, CENTER_Y - 85, 0, 0, 0x202020);
    std::vector<std::string> ms(menuString, menuString + 2);
    int menu = CommonMenu2(CENTER_X - 49, CENTER_Y - 50, 48, ms);
...
void instruct_15()
{
    ...
    std::string str1 = " \xe5\xb0\x8f\xe6\x9d\x91\xe7\x9a\x84\xe5\x82\xb3\xe8\xaa\xaa\xe5\xa4\xb1\xe6\x95\x97\xe4\xba\x86\xe2\x80\xa6"; // 小村的傳說失敗了…
    DrawShadowText(str1.c_str(), CENTER_X - 120, CENTER_Y - 25, ColColor(255), ColColor(255));
    std::string str2 = "\xe4\xbd\x86\xe6\x98\xaf\xe9\x81\x8a\xe6\x88\xb2\xe6\x98\xaf\xe5\x8f\xaf\xe4\xbb\xa5\xe9\x87\x8d\xe4\xbe\x86\xe7\x9a\x84\xef\xbc\x81"; // 但是遊戲是可以重來的！
...
void instruct_33(int rnum, int mnum, int dismode)
{
    ...
    if (dismode == 0)
    {
        std::string word = "\xe5\xad\xb8\xe6\x9c\x83"; // 學會
...
void instruct_34(int rnum, int iq)
{
    ...
    if (iq > 0)
    {
        std::string word = "\xe8\xb3\x87\xe8\xb3\xaa\xe5\xa2\x9e\xe5\x8a\xa0"; // 資質增加
        char buf[16];
...
void instruct_45(int rnum, int speed)
{
    ...
    Show3HintString(Rrole[rnum].Name, "\xe8\xbc\x95\xe5\x8a\x9f\xe5\xa2\x9e\xe5\x8a\xa0", buf);
}
...
void instruct_46(int rnum, int mp)
{
    ...
    Show3HintString(Rrole[rnum].Name, "\xe5\x85\xa7\xe5\x8a\x9b\xe5\xa2\x9e\xe5\x8a\xa0", buf);
}
...
void instruct_47(int rnum, int Attack)
{
    ...
    Show3HintString(Rrole[rnum].Name, "\xe6\xad\xa6\xe5\x8a\x9b\xe5\xa2\x9e\xe5\x8a\xa0", buf);
}
...
void instruct_48(int rnum, int hp)
{
    ...
    Show3HintString(Rrole[rnum].Name, "\xe7\x94\x9f\xe5\x91\xbd\xe5\xa2\x9e\xe5\x8a\xa0", buf);
}
...
void instruct_52()
{
    ...
    ShowRolePro(0, idx, "\xe4\xbd\xa0\xe7\x9a\x84\xe5\x93\x81\xe5\xbe\xb7\xe6\x8c\x87\xe6\x95\xb8\xe7\x82\xba\xef\xbc\x9a"); // 你的品德指數為：
}
...
void instruct_53()
{
    ...
    ShowRolePro(0, idx, "\xe4\xbd\xa0\xe7\x9a\x84\xe8\x81\xb2\xe6\x9c\x9b\xe6\x8c\x87\xe6\x95\xb8\xe7\x82\xba\xef\xbc\x9a"); // 你的聲望指數為：
}
...
void ShowTeamMate(int position, int headnum, int Count)
{
    ...
    std::string str = std::string(Star[position]) + " " + RoleName[position] + " \xe6\x88\x90\xe7\x82\xba\xe5\xa4\xa5\xe4\xbc\xb4"; // 成為夥伴
...
void MissionList(int mode)
{
    ...
    const char* missiontip[3] = {
        "\xe6\x9c\xaa\xe9\x96\x8b\xe5\x95\x9f", // 未開啟
        "\xe6\x9c\xaa\xe5\xae\x8c\xe6\x88\x90", // 未完成
    ...
    {
        std::vector<std::string> ms = {"\xe6\xb2\x92\xe6\x9c\x89\xe7\xac\xa6\xe5\x90\x88\xe7\x9a\x84\xe4\xbb\xbb\xe5\x8b\x99"}; // 沒有符合的任務
...
void RoleEnding(int starnum, int headnum, int talknum)
{
    ...
    std::string str2;
    if (status == 0) str2 = "\xef\xbc\x9f\xef\xbc\x9f\xef\xbc\x9f"; // ？？？
    else
    {
...
void SetAttribute(int rnum, int selecttype, int modlevel, int minlevel, int maxlevel)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        int magichurt = Rmagic[mnum].Attack_[0] + (Rmagic[mnum].Attack_[1] - Rmagic[mnum].Attack_[0]) * magiclevel / 10;
...
void NewShop(int shop_num)
{
    ...
    std::string shopStr = "\xe9\x9c\x80\xe8\xa6\x81\xe8\xb2\xb7\xe4\xbb\x80\xe9\xba\xbc\xef\xbc\x9f"; // 需要買什麽？
    ...
    while (!sure)
    {
        ...
        char moneyBuf[64];
        snprintf(moneyBuf, sizeof(moneyBuf), "\xe7\x8f\xbe\xe6\x9c\x89\xe9\x8a\x80\xe5\x85\xa9\xef\xbc\x9a%5d", money);
        DrawTextWithRect(moneyBuf, x, y + 140, 160, 0, 0x202020, 0, 0);
        snprintf(moneyBuf, sizeof(moneyBuf), "\xe8\x8a\xb1\xe8\xb2\xbb\xe4\xbc\xb0\xe7\xae\x97\xef\xbc\x9a%5d", totalprice);
...
int16_t EnterNumber(int MinValue, int MaxValue, int x, int y, int Default)
{
    ...
    // 绘制基本界面
    char buf[64];
    snprintf(buf, sizeof(buf), "\xe7\xaf\x84\xe5\x9c\x8d%d~%d", MinValue, MaxValue); // 範圍
...
bool EnterString(std::string& str, int x, int y, int w, int h)
{
    ...
    while (true)
    {
        tick++;
        LoadFreshScreen();
        std::string display = "\xe8\xab\x8b\xe8\xbc\xb8\xe5\x85\xa5\xe4\xb8\xbb\xe8\xa7\x92\xe4\xb9\x8b\xe5\xa7\x93\xe5\x90\x8d\xef\xbc\x9a" + str; // 請輸入主角之姓名：
